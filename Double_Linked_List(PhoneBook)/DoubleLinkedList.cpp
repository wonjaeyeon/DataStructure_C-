...
• #include <cstring> 추가
• 함수 내에서 문자열 복사는 strcpy() 함수를 사용
	‣ 함수원형 참고 : char* strcpy(char* dest, const char* origin);
• 함수 내에서 문자열 비교는 strcmp() 함수를 사용
	‣ 함수원형 참고 : int strcmp(const char *s1, const char *s2);
*/
void DLinkedList::Add(char name[], char phone[])
{
	NODE *temp = new NODE;

	strcpy_s(temp->Data.name, name);
	strcpy_s(temp->Data.phone, phone);
	...
	*/
	bool exist = Exist(name, phone);
	if (exist == true)
	{
		cout << "입력한 이름 : " << name << ", 전화번호 : " << phone << "은 이미 리스트에 등록되어 있습니다." << endl;
		return;
	}
...
void DLinkedList::Remove(char phone[])
{
	...
	NODE *temp = Head;
	NODE *prev = NULL;
	char* tempb = temp->Data.phone;
	// 삭제 대상 노드를 찾음
	while (temp != NULL && strcmp(tempb, phone)==0)
	{
		prev = temp;
		temp = temp->Next;
	}
...
*/
bool DLinkedList::Exist(char name[], char phone[])
{
	...
	while (temp != NULL)
	{
		if (strcmp(temp->Data.name,name) == 0 && strcmp(temp->Data.phone, phone) == 0) // 리스트의 요소를 name,phone값과 하나씩 비교한다.
		{
